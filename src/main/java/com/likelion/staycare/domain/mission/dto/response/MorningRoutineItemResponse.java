...
public record MorningRoutineItemResponse(
        @Schema(description = "고정 아침 미션 항목 ID. 삭제 API에서 itemId로 사용", example = "12")
        Long itemId,

        @Schema(description = "매일 반복 수행할 미션 문구", example = "외출 전 자외선 차단제를 꼼꼼하게 바르기")
        String content,

        @Schema(description = "미션 category 라벨. CUSTOM 또는 SURVEY 항목은 null 가능", example = "자외선 차단", nullable = true)
        String category,

        @Schema(description = "미션 출처. AI 추천, 생활 루틴 후보 선택(SURVEY), 직접 입력(CUSTOM) 중 하나입니다.", example = "AI")
